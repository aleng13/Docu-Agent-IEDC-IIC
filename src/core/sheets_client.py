...
def find_event_row(service, sheet_id: str, event_name: str, config: dict) -> int | None:
    ...
    try:
        ...
        result = (
            ...
            .values()
            .get(spreadsheetId=sheet_id, range=range_to_read)
            .execute()
        )
        ...
        for i, row in enumerate(values):
            ...
            if normalized_search in normalized_cell or normalized_cell in normalized_search:
                ...
                log.info(
                    f"Found '{event_name}' matched '{cell_value}' at row {actual_row_index}"
                )
...
def update_event_row(service, sheet_id: str, row_index: int, data: dict, config: dict) -> bool:
    ...
    Instead of building a full row, this updates each mapped cell individually.
    Missing or empty values are written as "-" to avoid blank cells.
    """
    try:
        if not config:
            raise ValueError("Config is None")
        ...
        for field_name, col_letter in column_map.items():
            ...
            if raw_value is None:
                ...
                value = normalized_value if normalized_value else "-"
